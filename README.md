...
in order to make a simple server take in two numbers and addition them, you need to declare a custom interface for "ros messages",
stored in a ".srv" file, as such
```
int64 a
int64 b
---
...
then there is some black magic involving cmake and xml, which I won't get into for now.
...
```bash
ros2 run robot_control addition_server
```

and then making requests from another terminal with
```bash
ros2 service call /add_two_ints /robot_interfaces/srv/AddTwoInts "{a=5;b=7}"
```

the server will reply with AddTwoInts_Response(sum=12).

of course, the next step is writing an actual client node.
